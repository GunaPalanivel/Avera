...
def generate():
    ...
    with data_path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if "marketing" in title and len(selected) < 5:
                selected.append((c["candidate_id"], 20 - len(selected), "Marketing Manager honeypot trap, automatically disqualified."))
    ...
    batch = {
        ...
        "notes": [
            {"candidate_id": s[0], "rank": s[1], "reason": s[2]}
            for s in sorted(selected, key=lambda x: x[1])
        ]
    }
