...
def create_calibration_fixture():
    with open("calibration_samples.json", encoding="utf-8") as f:
        samples = json.load(f)
    ...
    groups = {"strong_ai": [], "tcs_infosys": [], "bad_behavior": [], "marketing_honeypot": [], "fictional_company": []}
    ...
    add_group(
        groups["tcs_infosys"],
        "{id} is an AI Engineer but has a consulting-only background (TCS/Infosys), which is a soft disqualifier.",
        5,
    )
    add_group(
        groups["bad_behavior"],
        "{id} has AI experience but terrible behavioral signals (very low response/acceptance rate).",
        9,
    )
    add_group(
        groups["marketing_honeypot"],
        "{id} is a Marketing Manager with AI keywords. This is a honeypot and must be disqualified.",
        13,
    )
    add_group(
        groups["fictional_company"],
        "{id} works at a fictional company. Must be dropped by the first pipeline stage.",
        17,
    )
