...
**Not shipped:** duration-weighted skill repetition in `build_candidate_text()` and aspiration phrase penalties — both regressed calibration metrics on the full 100K run despite passing unit tests.
...
- Deferred: duration-weighted embeddings and aspiration detection need safer calibration before reintroduction.
