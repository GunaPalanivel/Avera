...
Add a **Semantic Scorer** (27% base weight for senior JDs; see ADR-019 for weight rebalance) using `sentence-transformers` (`all-MiniLM-L6-v2`):
...
4. **Deterministic scorers** cover the remaining base score (title/career 18%, skills 14%, trajectory 16%, education 8%, experience 11%, location 6% for senior JDs) for bounded, auditable constraints. Pure keyword scorers (title + skills = 32%) sit below semantic + trajectory combined.
