...
1. **Pass 1 — `prefill_semantic_stream()`**: stream candidates; compute the heuristic score (title + skills + experience + location) for all; keep the survivors above `SEMANTIC_MIN_HEURISTIC_SCORE` (**0.11**), then batch-encode only the strongest `SEMANTIC_RERANK_TOPK` (default **5000**, deterministic selection). Batch size is `AVERA_SEMANTIC_BATCH` (default 128). Results cached by `candidate_id`.
